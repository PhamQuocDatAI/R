...
import time
from ..spiders import define


url = 'https://oto.com.vn/mua-ban-xe'
# Get link trademark
link_trade = define.get_link_trademark(url)
# Get name trademark, link style, name style, link product
name_trade = []
link_style = []
name_style = []
for i in link_trade:
    for j in define.get_name_style_car(i):
        name_style.append(j)
    name_trade.append(define.get_name_trademark(i))
    for k in define.get_link_style_car(i):
        link_style.append(k)
