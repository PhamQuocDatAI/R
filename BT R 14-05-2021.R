#Download file RIKZ.txt trên Google ClassRoom v???
#Sample C1 P1 P2 P3 P4 P5 P6 P7 P8 P9 P10
#P11 P12 P13 P14 P15 P16 P17 P18 P19 P20 P21 P22 P24
#P25 N1 CR1 CR2 CR3 CR4 CR5 CR6 CR7 CR8 CR9 CR10
#CR11 CR12 CR13 CR14 CR15 CR16 CR17 CR18 CR19
#CR20 CR21 CR22 CR23 CR24 CR25 CR26 CR27 CR28
#M1 M2 M3 M4 M5 M6 M7 M8 M9 M10 M11 M12 M14 M15 M16
#M17 I1 I2 I3 I4 I5
#week angle1 angle2 exposure salinity
#temperature NAP penetrability grainsize


#Thi???t l???p thý m???c hi???n hành
setwd("C://Users//PC//Desktop//R")
#RIKZ.txt
Benthic <- read.table("RIKZ.txt", header = TRUE)
names(Benthic)
str(Benthic)
#Bi???n Species (loài) t??? c???t th??? 2 ð???n 76 trong bi???n Benthic
Species <- Benthic[,2:76]
names(Species)
str(Species)
#Ki???m tra dimensions - chi???u c???a bi???n Species
n <- dim(Species)
n
# 45 d??ng - týõng ???ng v???i 45 sites - v??? trí thu th???p m???u v???t
# Týõng ???ng v???i m???i v??? trí th?? s??? ghi nh???n ðý???c nh???ng loài
#sinh v???t bi???n nào?
#Có bao nhiêu loài sinh v???t bi???n ??? v??? trí s??? 1
sum(Species[1,], na.rm = TRUE)
#K???t qu??? 143 loài sinh v???t bi???n xu???t hi???n ??? site #1
sum(Species[2,], na.rm = TRUE)
#... Làm 45 l???n ð??? ki???m tra s??? lý???ng sinh v???t bi???n
#xu???t hi???n t???i các sites
TA <- vector(length = n[1])
for (i in 1:n[1]) {
  TA[i] <- sum(Species[i,], na.rm = TRUE)
}
TA
#Hàm rowSums: tính toán t???ng c???a các c???t ??? trong hàng
#tham s??? na.rm=TRUE => Lo???i b??? các giá tr??? r???ng


TA02 <- rowSums(Species, na.rm = TRUE)
TA02
#Ði vào chi ti???t t???i t???ng v??? trí quan sát
sum(Species[1,]>0, na.rm = TRUE)
#K???t qu??? chúng ta có 11 loài khác nhau xu???t hi???n t???i v??? trí 1
Richness <- vector(length=n[1])
for (i in 1:n[1]) {
  Richness[i] <- sum(Species[i,]>0, na.rm=TRUE)
}
Richness


Richness02 <- rowSums(Species>0, na.rm = TRUE)
Richness02


# Ch??? s??? ða d???ng loài
#https://en.wikipedia.org/wiki/Diversity_index
#T??m hi???u cách tính ch??? s??? ða d???ng loài
#10h40.
#H - sum bi???n ch???y t??? i -> m c???a bi???n p: xác xu???t * log10 p
RS <- rowSums(Species, na.rm=TRUE)
prop <- Species / RS
H <- -rowSums(prop * log10(prop), na.rm= TRUE)
H
# K???t h???p l???i h???t nh???ng g?? ð?? làm vô 1 hàm c???a riêng m??nh
# sau này s??? d???ng l???i ð??? phân tích nh???ng d??? li???u týõng t???
Index.function <-function(Spec, Choice){
  if (Choice=="Richness") {
    Index <- rowSums(Species>0, na.rm = TRUE)
  }
  if (Choice=="Total Abundance") {
    Index <- rowSums(Species, na.rm = TRUE)
  }
  if (Choice=="Shannon") {
    RS <- rowSums(Species, na.rm=TRUE)
    prop <- Species / RS
    Index <- -rowSums(prop * log10(prop), na.rm= TRUE)
  }
  list(Index=Index, MyChoice= Choice)
}

